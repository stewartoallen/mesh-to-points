...
typedef struct {
    ...
    // Precomputed normal (for backface culling)
    Vec3 normal;
} Triangle;
...
typedef struct {
    GridCell* cells;        // 3D array of cells (flattened)
    int res_x, res_y, res_z; // Grid resolution
    float cell_size_x, cell_size_y, cell_size_z; // Cell dimensions
    Vec3 grid_min;          // Grid origin
} UniformGrid;
...
// Helper: get grid cell index from 3D coordinates
inline int get_cell_index(UniformGrid* grid, int x, int y, int z) {
    return z * (grid->res_x * grid->res_y) + y * grid->res_x + x;
}

// Helper: add triangle to grid cell
void add_triangle_to_cell(GridCell* cell, int tri_index) {
    ...
    cell->triangle_indices[cell->count++] = tri_index;
}

// Build uniform grid acceleration structure
UniformGrid* build_grid(Triangle* triangles, int triangle_count, BoundingBox bounds) {
    UniformGrid* grid = (UniformGrid*)malloc(sizeof(UniformGrid));

    // Determine grid resolution (aim for ~20-30 cells per dimension)
    float size_x = bounds.max.x - bounds.min.x;
    float size_y = bounds.max.y - bounds.min.y;
    float size_z = bounds.max.z - bounds.min.z;

    float avg_size = (size_x + size_y + size_z) / 3.0f;
    int target_res = 25;

    grid->res_x = (int)(size_x / avg_size * target_res) + 1;
    grid->res_y = (int)(size_y / avg_size * target_res) + 1;
    grid->res_z = (int)(size_z / avg_size * target_res) + 1;

    // Clamp resolution
    if (grid->res_x > 100) grid->res_x = 100;
    if (grid->res_y > 100) grid->res_y = 100;
    if (grid->res_z > 100) grid->res_z = 100;
    if (grid->res_x < 1) grid->res_x = 1;
    if (grid->res_y < 1) grid->res_y = 1;
    if (grid->res_z < 1) grid->res_z = 1;

    grid->cell_size_x = size_x / grid->res_x;
    grid->cell_size_y = size_y / grid->res_y;
    grid->cell_size_z = size_z / grid->res_z;

    grid->grid_min = bounds.min;

    // Allocate cells
    int total_cells = grid->res_x * grid->res_y * grid->res_z;
    grid->cells = (GridCell*)calloc(total_cells, sizeof(GridCell));

    // Assign triangles to cells based on their bounding boxes
    for (int t = 0; t < triangle_count; t++) {
        Triangle* tri = &triangles[t];

        // Get cell range for this triangle
        int min_cell_x = (int)((tri->bbox_min_x - grid->grid_min.x) / grid->cell_size_x);
        int max_cell_x = (int)((tri->bbox_max_x - grid->grid_min.x) / grid->cell_size_x);
        int min_cell_y = (int)((tri->bbox_min_y - grid->grid_min.y) / grid->cell_size_y);
        int max_cell_y = (int)((tri->bbox_max_y - grid->grid_min.y) / grid->cell_size_y);

        // For Z, use full triangle bbox
        float tri_min_z = tri->v0.z;
        float tri_max_z = tri->v0.z;
        if (tri->v1.z < tri_min_z) tri_min_z = tri->v1.z;
        if (tri->v2.z < tri_min_z) tri_min_z = tri->v2.z;
        if (tri->v1.z > tri_max_z) tri_max_z = tri->v1.z;
        if (tri->v2.z > tri_max_z) tri_max_z = tri->v2.z;

        int min_cell_z = (int)((tri_min_z - grid->grid_min.z) / grid->cell_size_z);
        int max_cell_z = (int)((tri_max_z - grid->grid_min.z) / grid->cell_size_z);
        ...
        if (max_cell_z >= grid->res_z) max_cell_z = grid->res_z - 1;
        ...
        for (int z = min_cell_z; z <= max_cell_z; z++) {
            for (int y = min_cell_y; y <= max_cell_y; y++) {
                for (int x = min_cell_x; x <= max_cell_x; x++) {
                    int cell_idx = get_cell_index(grid, x, y, z);
...
float* convert_to_point_mesh(float* triangles, int triangle_count, float step_size, int* out_point_count) {
    ...
    for (int i = 0; i < triangle_count; i++) {
        ...
        // Precompute triangle normal for backface culling
        // Normal = (v1 - v0) × (v2 - v0)
        Vec3 edge1, edge2;
        ...
        tris[i].normal.x = edge1.y * edge2.z - edge1.z * edge2.y;
        tris[i].normal.y = edge1.z * edge2.x - edge1.x * edge2.z;
        tris[i].normal.z = edge1.x * edge2.y - edge1.y * edge2.x;
    }

    // Build acceleration grid
    UniformGrid* grid = build_grid(tris, triangle_count, bounds);

#if PRINTF_DEBUG
    // Debug: print grid statistics
    printf("Grid: %dx%dx%d = %d cells\n", grid->res_x, grid->res_y, grid->res_z,
           grid->res_x * grid->res_y * grid->res_z);
    int total_refs = 0;
    int max_tris_per_cell = 0;
    for (int i = 0; i < grid->res_x * grid->res_y * grid->res_z; i++) {
        total_refs += grid->cells[i].count;
        if (grid->cells[i].count > max_tris_per_cell) {
            max_tris_per_cell = grid->cells[i].count;
        }
    }
    printf("Total triangle references: %d\n", total_refs);
    printf("Avg triangles per cell: %.1f\n", (float)total_refs / (grid->res_x * grid->res_y * grid->res_z));
    printf("Max triangles in a cell: %d\n", max_tris_per_cell);
#endif
    ...
    for (float x = bounds.min.x; x <= bounds.max.x; x += step_size) {
        for (float y = bounds.min.y; y <= bounds.max.y; y += step_size) {
            ...
            // Determine which XY cell the ray passes through
            int cell_x = (int)((x - grid->grid_min.x) / grid->cell_size_x);
            int cell_y = (int)((y - grid->grid_min.y) / grid->cell_size_y);
            ...
            // Use a simple array to track which triangles we've already tested for this ray
            // (to avoid duplicate tests when triangles span multiple Z cells)
            static int tested_flags[10000]; // Assumes max 10k triangles
            static int test_stamp = 0;
            test_stamp++;

            // Traverse Z cells along the ray
            for (int cell_z = 0; cell_z < grid->res_z; cell_z++) {
                int cell_idx = get_cell_index(grid, cell_x, cell_y, cell_z);
                GridCell* cell = &grid->cells[cell_idx];

                // Test only triangles in this cell (skip if already tested)
                for (int i = 0; i < cell->count; i++) {
                    int tri_idx = cell->triangle_indices[i];

                    // Skip if we've already tested this triangle for this ray
                    if (tri_idx < 10000 && tested_flags[tri_idx] == test_stamp) {
                        continue;
                    }
                    if (tri_idx < 10000) {
                        tested_flags[tri_idx] = test_stamp;
                    }

                    // Backface culling: skip if normal points away from ray (+Z)
                    // For upward rays, only test triangles with normal.z > 0
                    if (tris[tri_idx].normal.z <= 0) {
                        continue;
                    }

                    Vec3 intersection;
                    if (ray_triangle_intersect(ray_origin, ray_dir, &tris[tri_idx], &intersection)) {
