...
// Include v2 generator
#define HeightMap HeightMapV2
#define ToolPath ToolPathV2
#include "../wasm/toolpath-generator-v2.c"
#undef HeightMap
#undef ToolPath
...
int main(int argc, char** argv) {
    ...
    // Create height maps
    printf("\n--- Creating Height Maps (V2) ---\n");
    start = clock();
    HeightMapV2* terrain_map = create_terrain_map(terrain_points, terrain_point_count, step_size);
    HeightMapV2* tool_map = create_tool_map(tool_points, tool_point_count, step_size);
    ...
    start = clock();
    ToolPathV2* path = generate_toolpath(terrain_map, tool_map, x_step, y_step, -100.0f);
